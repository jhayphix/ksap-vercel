  ...
  const getApplicationsTableData = useCallback(
    (
      ...
      const applicationTableData = filteredData?.map((application) => {
        // Flatten the responses into a single object with a "z_z" indicator in the label
        const responses = application?.responseSections?.reduce(
          (acc, section) => {
            section.responses?.forEach((response) => {
              // Adding "z_z" to the label (you could change "z_z" to any other appropriate term)
              const labelWithIndicator = `z_z ${response.label}`;
